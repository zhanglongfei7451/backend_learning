...
func TestMetadata(t *testing.T) {
	...
	for i, _test := range tests {
		geoIP, err := newGeoIP("D:\\1—SUYAN\\DNS\\Ecloud-DNS\\fileagent\\ECLOUD-DNS\\coredns-updater\\mmdb\\custom_lines.mmdb")
		if err != nil {
			t.Fatalf("Test %d: unable to create geoIP plugin: %v", i, err)
		}
		state := request.Request{
			W: &test.ResponseWriter{RemoteIP: "1.1.1.1"}, // This IP should be be part of the CDIR address range used to create the database fixtures.
		}
